...
def add_globals_to_file(commanderName):
    ...
    with open(path.join("commanders", f"{commanderName}", f"{commanderName}.py"), encoding='utf-8') as f:
        lines = f.readlines()
        for line in lines:
            original_lines.append(line)

    with open(path.join("commanders", f"{commanderName}", f"{commanderName}.py"), "w", encoding='utf-8') as f:
